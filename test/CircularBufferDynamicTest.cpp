...
TYPED_TEST(CircularBufferDynamicTest, ValidSpanCreation) {
    for (int i = 0; i < 5; ++i) {  // Less than capacity
        this->buffer.push(i);
    }
...
TYPED_TEST(CircularBufferDynamicTest, EmptySpanCreation) {
    auto span = this->buffer.span();
    EXPECT_FALSE(span.has_value());
}

// Test span creation on wrap-around
TYPED_TEST(CircularBufferDynamicTest, WrapAroundSpanCreation) {
